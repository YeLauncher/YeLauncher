...
void main() {
  ...
  late MockInstanceStylingRepository mockInstanceStylingRepository;
  ...
  setUp(() async {
    ...
    provideDummy<IconData>(Icons.inventory_2_rounded);
    provideDummy<Color>(const Color(0xFFFFFFFF));
    mockInstanceStylingRepository = MockInstanceStylingRepository();
    mockToastService = MockToastService();
    notificationProvider = NotificationProvider();

    when(
      mockInstanceStylingRepository.getIconData(any),
    ).thenReturn(Icons.inventory_2_rounded);
    when(
      mockInstanceStylingRepository.getColor(
        any,
        fallback: anyNamed('fallback'),
      ),
    ).thenAnswer(
      (Invocation invocation) =>
          invocation.namedArguments[Symbol('fallback')] as Color,
    );
    ...
    return MultiProvider(
      providers: [
        ...
        Provider<InstanceStylingRepository>.value(
          value: mockInstanceStylingRepository,
        ),
        Provider<ToastService>.value(value: mockToastService),
  ...
  testWidgets(
    'updates mod by deleting old jar and replacing in installedContent',
    (WidgetTester tester) async {
      ...
      final modDir = Directory(
        p.join(tempDir.path, 'instances', instanceId, 'mods'),
      );
      ...
      when(
        mockContentRepository.getContent(any),
      ).thenAnswer((_) async => Result.success(item));
      when(
        mockContentRepository.getVersions(any),
      ).thenAnswer((_) async => const Result.success([]));
      when(
        mockContentRepository.getProjectDependencies(any),
      ).thenAnswer((_) async => const Result.success([]));
      ...
      final captured = verify(
        mockInstanceRepository.saveInstance(captureAny),
      ).captured;
